...
import { Search, MessageSquare, Send, Copy } from "lucide-react-native";
...
export default function ClientMessages() {
  ...
  const { id: conversationId, name } = params;
  ...
  const renderConversationItem = ({ item }: { item: Conversation }) => (
    <TouchableOpacity
      ...
      onPress={() =>
        router.push({
          pathname: "/messages",
          params: { id: item.user_id, name: item.username },
        })
      }
    >
      <View style={styles.conversationInfo}>
        <Text style={styles.conversationName}>{item.username}</Text>
        <Text style={styles.userIdText}>ID: {truncateId(item.user_id)}</Text>
      ...
      <SafeAreaView style={styles.container}>
        <View style={styles.messageHeader}>
          ...
          <View style={styles.headerTitleContainer}>
            <Text style={styles.messageTitle}>{name || "Professional"}</Text>
            
            <TouchableOpacity
              onPress={() => copyToClipboard(conversationId as string)}
              style={styles.copyButton}
            >
              <Text style={styles.conversationIdText}>
                ID: {truncateId(conversationId as string)}
              </Text>
              <Copy size={14} color="#9CA3AF" style={styles.copyIcon} />
              {copied && (
      ...
      {conversations.length > 0 ? (
        ...
        <View style={styles.emptyState}>
          ...
          <Text style={styles.emptyDescription}>
            Start a conversation with a professional to see messages here
          </Text>
        </View>
      )}
    </SafeAreaView>
